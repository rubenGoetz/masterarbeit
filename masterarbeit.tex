...
With this, our approach is able to solve 304 of our 400 instances within a time limit of 300 seconds and achieves a geometric mean speedup of 10.260 over the state of the art serial SAT solver Kissat~\cite{kissat}. In our direct comparison to the state of the art parallel SAT solver MallobSat~\cite{mallobSat} (in its default search-only configuration), we observe a significant decrease in memory consumption, albeit with a not insignificant runtime increase for most instances. Our approach's geometric mean of the geometric mean memory consumption, over the course of an instance's runtime, is between 0.282 and 0.216 when compared to MallobSat. For some instances, our approach even consumes more than an order of magnitude less memory than MallobSat.

\subsection{Structure of the Thesis}

We structure this thesis as follows: Section~\ref{sec:preliminaries} contains essential definitions and provides an overview of SAT solving. This includes serial SAT solving, parallel SAT solving in Section~\ref{sec:parallelSat}, and previous efforts to increase memory-efficiency in SAT solvers in Section~\ref{sec:memEfficientSat}. Furthermore, we introduce the two SAT solvers MallobSat in Section~\ref{sec:mallob} and Gimsatul in Section~\ref{sec:gimsatul} -- two essential building blocks of our work.
Section~\ref{sec:approach} gives a detailed description of the implementation of our new approach, which is then evaluated based on our experiments in Section~\ref{sec:expEval}. This includes a detailed account of our achieved speedups in Section~\ref{sec:speedups} and a comparison to MallobSat in Section~\ref{sec:compare}. Finally, we draw our conclusions in Section~\ref{sec:conclusion}.
...
In the following, we first give some preliminary definitions that are used throughout this work in Section~\ref{sec:definitions}. Then, we give an overview of some related work including research into SAT solving in general, parallel SAT solving, and attempts to reduce memory consumption in parallel SAT solvers in Section~\ref{sec:relatedWork}.
...
In the following, we use the term \textit{SAT solver} for a program that is designed to solve the previously described SAT problem on concrete instances, usually given in the \textit{DIMACS CNF file} format.
...
We do want to describe Gimsatul's clause sharing mechanisms in more detail, however, since this will become important later on. The process is depicted in Figure~\ref{fig:architectureGimsatul}. If a ring derives a clause it deems worthy to share (i.e., a clause with a low glue value) it will store a reference to this clause inside a buffer of small constant size for each other ring. If a buffer is full, it will overwrite the clause with the highest glue value in the buffer. If all clauses in the buffer have a lower glue value than the currently exported clause, it will not be inserted. When a ring decides to import a clause, it will pick another ring at random and import the clause with the lowest glue value in it's respective buffer -- deleting the clause from the buffer in the process.
...
To diversify its rings, Gimsatul utilizes differing strategies for the first local search of each ring. A strategy consists of the restart strategy, the initial phases, and whether or not reason bumping is activated. Possible restart strategies are \textit{focused mode}, \textit{stable mode}, or a combination of the two as described by Biere and Fleury~\cite{restartStrategy}. Initial phases are set either to 1 or 0 for all variables and reason bumping is either activated or deactivated. This results in up to 12 differing strategies. Again, more detail is given in the original article~\cite{gimsatul}.
...
In this section, we describe our approach and important design decisions. We integrated Gimsatul into MallobSat as a solver backend to utilize both Gimsatul's memory-efficiency, achieved by shared memory clause sharing, and MallobSat's massively parallel scalability. We also want to describe the architecture that we used to make it possible to use the already parallel SAT solver Gimsatul as a solver backend in MallobSat.

\subsection{General Architecture}

We now describe the general architecture of our approach, integrating Gimsatul (with its memory-efficient shared memory approach) as a solver engine into MallobSat. In the following, we will use \textit{external} and \textit{internal} when talking about our architecture w.r.t. MallobSat from the point of view of a Gimsatul instance. For example, when rings of a Gimsatul instance exchange derived clauses, we will denote this as \text{internal clause exchange}, if a Gimsatul instance is exchanging clauses as a solver engine in MallobSat we will call this \textit{external clause exchange}.
...
First off, Gimsatul is initialized by reading the original CNF formula of a given Problem, running some preprocessing, initializing a single ring and then cloning said ring $t - 1$ times to create its $t$ solving threads (i.e., rings). If we integrated each ring to be a solver engine in MallobSat, we would have to reimplement all of Gimsatul's initialization to create each ring independently. This would make it unfeasible to keep Gimsatul's shared memory clauses. Additionally, MallobSat's clause sharing operation would double Gimsatul's internal clause sharing mechanisms -- potentially filtering important clauses from other solver engines in favor of already internally shared clauses from the same Gimsatul instance.
...
MallobSat assumes its solver engines to be serial blackbox SAT solvers, leaving us to redesign its engine setup to adopt a multi-threaded Gimsatul instance as a solver engine as follows: 
When initializing a process with $t$ solver engines (i.e., threads), MallobSat creates a portfolio of $t$ solvers from a given regular expression that defines the portfolio for the execution of MallobSat. If Gimsatul is part of this portfolio, we adapt this procedure to accumulate all $t' \leq t$ threads that are defined to be Gimsatul threads into a single solver engine and then run Gimsatul with $t'$ threads. The MallobSat process now contains $t - t' + 1$ solver engines and operates as if it ran with $t - t' + 1$ threads -- one of them being a Gimsatul instance.
...
Importing external clauses into Gimsatul is a bit more complex, however. Since we integrated Gimsatul as a single solver engine into MallobSat, we have to work with a single import buffer. MallobSat's import buffers are built in a way that clauses are immediately deleted when read by the solver engine. We want to reason our approach by first discussing two scrapped approaches.

\subsubsection{Rings Import Independently}

To make it possible for every ring to read the import buffer independently, we would have to redesign both MallobSat's import buffers and Gimsatul's clause lifecycle. The import buffers would need some functionality to keep track of which rings had already read which clauses. And Gimsatul's rings would need some functionality to import only the reference to a clause, if it had already been imported into the shared memory by another ring.

\subsubsection{Single Dedicated Importing Ring}
\label{sec:singleDedicatedImportRing}

Having a dedicated ring to import all clauses from the import buffer whenever it does a restart, leaves the challenge of spreading the imported external clauses throughout all of Gimsatul's rings. This approach already results in some scalability improvements over plain Gimsatul. However, since Gimsatul's internal clause sharing utilizes buffers of extremely limited size, importing the full import buffer from MallobSat all at once, and immediately exporting them internally in Gimsatul, would discard many of the external clauses due to internal buffer overflows.
...
\textbf{Number of Rings.} The number of rings in a Gimsatul instance is used as the limit on importing external clauses.
...
In our experiments, we found the three strategies \textit{Mean Free Slots}, \textit{Number of Rings}, and \textit{Constant} to perform similarly and the remaining two strategies \textit{Minimum Free Slots} and \textit{Secondary Minimum Free Slots} notably worse.
...
We found a significant increase in performance due to this practice and therefore employ it throughout all of our experiments. The comparison between our search-only option and the activated pre- and inprocessing is shown in Figure~\ref{fig:searchOnlyBoost}. Our search-only option solves 28 more instances within our time limit of 300 seconds. For our import strategy (as discussed in Section~\ref{sec:ourApproach}), we choose the \textit{Number of Rings} strategy, since it performed best in testing.
...
In this section, we first discuss our findings on runtime with respect to scalability on up to 16 nodes (i.e., 768 cores). We then compare our approach to the state of the art distributed SAT solver MallobSat, using its default configuration, utilizing one Kissat instance per core.
...
To calculate speedups, we ran the sate of the art serial SAT solver Kissat~\cite{kissat} for up to 3000 seconds on a single core. The results of this run are shown in Figure~\ref{fig:runtimeSerial}. The geometric means of all speedups for benchmark instances solved by Kissat are shown in Table~\ref{tab:speedups}. We observe increasing speedups with an increasing number of cores. The specific speedups, for all instances that were solved by Kissat within the 3000 second time limit, are shown in Figure~\ref{fig:speedups}. Here, we observe a significant variation in speedups for simple instances (i.e., instances Kissat solved in under 500 seconds). For complex instances (i.e., instances where Kissat needed more than 500 seconds), we see a tendency for the speedups to become greater. For very complex instances the speedups routinely beat the geometric mean while their variance increases even more.
...
Table~\ref{tab:speedupsSignificantFamilies} shows geometric mean speedups separated by significant families. We consider families to be significant if at least five instances contained in our benchmark set belong to this family. A complete table of all instance families is provided in the Appendix~\ref{app:speedupsFamiliesComplete}. The families are ordered by the geometric mean speedup over all five configurations.
...
Instances of the family \textit{minimum-disagreement-parity}~\cite{satComp2022}, encode the Minimum Disagreement Parity (MDP) problem, which is of interest to the cryptography community and results in challenging SAT problems. This is underlined by the small size of these instances: The largest one contains only 1021 variables, with the slowest solved by Kissat taking $2617.82\,$s and containing only 750 variables. This family comprises of three satisfiable and three unsatisfiable instances. Our high geometric mean speedups of over 22 suggest that the MDP problem instances benefit from the increased processing power of parallel sat solving far more than other kinds of problems.
...
\begin{table}[!h]
  ...
  \begin{tabular}{ lcccccc }
    ...
    \bottomrule
  \end{tabular}
  \caption{Geometric mean speedups per family and setup, for families comprising at least 5 instances and a calculatable speedup.}
...
We evaluated the \textit{penalized average runtime (PAR-2)} score, as it was used in the SAT Competition of 2024 to evaluate submitted solvers~\cite{satCompWebsite}. It is calculated by taking the average runtime over all instances, with unsolved instances included at a penalty of twice the time limit. The PAR-2 scores for MallobSat and our approach, for our time limit of 300 seconds, are listed in Table~\ref{tab:par2}. They are, however, not directly comparable to the results of the SAT Competition, since the competition uses a much larger time limit of 5000 seconds. The PAR-2 scores of our approach improve with an increasing number of cores, from 234.199 for 48 cores down to 176.129 for 768 cores.
...
\begin{figure}[!h]
  ...
  \begin{subfigure}[c]{.45\textwidth}
    ...
    \caption{1 node (48 cores)}
  \end{subfigure}
  \begin{subfigure}[c]{.45\textwidth}
    \center
    \includegraphics[scale=.45]{plots/4node_compare/mem_abs_over_vars.pdf}
    \caption{4 nodes (96 cores)}
  \end{subfigure}
  \begin{subfigure}[c]{.45\textwidth}
    \center
    \includegraphics[scale=.45]{plots/16node_compare/mem_abs_over_vars.pdf}
    \caption{16 nodes (768 cores)}
  \end{subfigure}
  \caption{Memory consumption over variables, using 1, 4, and 16 nodes.}
...
We now take a more detailed look at the memory consumption ratios. Figure~\ref{fig:memRatiosVars} shows the ratios between the peak memory usage of MallobSat and our approach for all our benchmark instances. We distinguish between solved and unsolved or partially solved instances and depict the 75th percentile of the ratios with a dashed line. The exact values of these 75th percentiles are provided in Table~\ref{tab:memRatioPercentiles}.
...
\begin{figure}[!h]
  ...
  \end{subfigure}
  \caption{Memory ratios between MallobSat and our approach, using 1, 4, and 16 nodes. Each data point corresponds to a benchmark instance.}
...
\begin{figure}[!h]
  ...
  \begin{subfigure}[c]{.45\textwidth}
    ...
    \caption{16 nodes (768 cores)}
  \end{subfigure}
  \caption{Geometric means for memory ratios between MallobSat and our approach per second, using 1, 4, and 16 nodes. Each data point corresponds to a benchmark instance.}
  \label{fig:memGmVars}
\end{figure}
...
First, we observe that for almost all instances the lines stay below a ratio of one. This indicates a systematic reduction in memory consumption for our approach, in comparison to MallobSat, during all stages of solving.
The second observation is the extreme jitters in some of our lines -- especially in the first few seconds of execution. We can explain this with a fairly simple effect of the search-only setting in our approach: At the beginning of the execution, we start a preprocessing thread in addition to the regular solving threads. This thread solely executes the preprocessing techniques implemented in Kissat. After the preprocessing is finished, the solver threads that are already running on the unprocessed instance, are gradually replaced by threads solving the preprocessed version of the problem formula. Since we collect a memory status every second, it is possible to get measurements while a thread is being restarted. An offset of one second between the two compared solvers, results in an extreme discrepancy in memory consumption -- thus causing the extreme jitters in our plots.
...
In this work, we integrate Gimsatul as a solver engine into the state of the art distributed SAT solver MallobSat, to bring Gimsatul's memory-efficiency to a distributed scale. We evaluate our approach in depth on up to 768 cores and find its performance to improve with an increasing numbers of cores.
...
\begin{longtable}{ lccccccc }
  ...
  \bottomrule
  \caption{Geometric mean speedups per family and setup.}
